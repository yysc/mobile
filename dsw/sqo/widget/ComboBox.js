define([
	...
	"dojox/mobile/sniff",
	"dojo/on"
], function(kernel, declare, lang, win, domGeometry, domStyle, domAttr, windowUtils, touch, AutoCompleterMixin, popup, ComboBoxMenu, TextBox, has, on){
//copy from dojox/mobile/ComboBox for fixing some issues on device.
//please search "//FIX BY" to find where are the changes.
	return declare("dsw.sqo.widget.ComboBox", [TextBox, AutoCompleterMixin], {
		// summary:
		...
		// FIX BY Lirui
		// viewPortHeight: [public] int
		//      The height of viewport for the device. this size will restrict the height of the dropdown.
		//      This values should be the viewport actual height subs the action bar height.
		viewPortHeight: -1,
		// ~END FIX
		...
		//FIX BY lirui: 
		//typeAheadDelay: INT
		//		time delay to trigger query request to backend. (millisecond)
		typeAheadDelay: 500,
		...
		openDropDown: function(){
			...
			//console.log("#in ComobBox.js# this.viewPortHeight:"+this.viewPortHeight);
			if(maxHeight == -1){
				// limit height to space available in viewport either above or below my domNode
				// (whichever side has more room)
				var viewport = windowUtils.getBox(),
					position = domGeometry.position(aroundNode, false);
				maxHeight = Math.floor(Math.max(position.y, (this.viewPortHeight == -1 ? viewport.h : this.viewPortHeight) - (position.y + position.h)));
			}
			...
			//FIX BY LIRUI: IF THERE IS MORE THAN ONE MARKUPS IN EACH ITEM, THEN THE OVERLAY WAS TOO HEIGHT.
			var childBox = domGeometry.position(dropDown.domNode.firstChild);
			if(childBox.h < maxHeight)
				dropDown.set("style",{height: childBox.h+1+"px"});
			//~END FIXED
			...
			if(wasClosed){
				...
				//FIX BY LIRUI: fix can not close dropdown when tapping out of combobox on device. change "mouseup" to touch.release
				this.endHandler = this.connect(win.doc.documentElement, touch.release,//"MOUSEUP",
					function(){
				...
				//FIX BY LIRUI: when user click dropdown item, then fire "itemSelect" event
				if(!this.itemChangeHandler){
					this.itemChangeHandler = on(dropDown, "change", lang.hitch(this, function(node){
						//this.itemChangeHandler.remove();
						this.onItemSelect(node);
					}));
				}
				//~END FIX
		...
		//FIX BY LIRUI: IF USER CLICK DROPDOWN ITEM, THEN FIRE "itemSelect" EVENT
		//Stub:
		onItemSelect: function(/*DomNode*/ node){
			//fired when menu item was selected
		}
		//~END FIX
	});
});
